...
class HostBackupParams {
  ...
  /// 派生是否放到独立 isolate 里跑。
  ///
  /// 生产要放：标准参数（N=32768）约 32 MiB 内存、纯 CPU 约 0.35 秒，
  /// 跑在主 isolate 上会把界面冻住。
...
Future<Uint8List> _deriveInIsolate(
  ...
  if (!params.useIsolate) {
    return Future.value(_deriveScryptSync(password, salt, params));
  }
