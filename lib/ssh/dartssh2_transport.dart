...
final class DartSsh2Transport implements SshTransport {
  ...
  Future<DuplexChannel> openDirectChannel(String host, int port) async {
    ...
    try {
      ...
      return _DartSsh2Channel(channel);
    } on Object catch (error) {
  ...
  Future<RemoteForwardListener> openRemoteForward({
    ...
    try {
      ...
      return _DartSsh2RemoteForward(client, forward);
    } on ForwardException {
      rethrow;
    } on Object catch (error) {
      throw dartSsh2ForwardError(error);
  ...
  Future<DynamicForwardProxy> openDynamicProxy({
    ...
    try {
      ...
      return _DartSsh2DynamicForward(forward);
    } on Object catch (error) {
      throw dartSsh2ForwardError(error);
    }
  }
...
class _DartSsh2Channel implements DuplexChannel {
  _DartSsh2Channel(this._channel);
...
/// 服务端监听型转发的适配。
class _DartSsh2RemoteForward implements RemoteForwardListener {
  _DartSsh2RemoteForward(this._client, this._forward);
  ...
  Stream<DuplexChannel> get connections =>
      _forward.connections.map(_DartSsh2Channel.new);
...
/// 本地 SOCKS5 代理的适配。
class _DartSsh2DynamicForward implements DynamicForwardProxy {
  _DartSsh2DynamicForward(this._forward);
