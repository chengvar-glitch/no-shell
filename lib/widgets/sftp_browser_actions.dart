...
/// 操作类错误文案：把分类结果翻成当前语言。
String sftpErrorText(AppLocalizations l10n, Object? error) {
...
/// 目录 / 文件图标：按扩展名粗分，够用即可。
IconData entryIcon(SftpEntry entry) {
...
Future<void> _upload(
  ...
  try {
    ...
    _toast(context, l10n.sftpPickerFailed);
    return;
  }
...
Future<void> _download(
  ...
  if (outcome == SftpDownloadOutcome.unavailable) {
    _toast(context, l10n.sftpTargetUnavailable);
  }
}
...
Future<void> _rename(
  ...
  try {
    ...
    if (!context.mounted) return;
    _toast(context, sftpErrorText(l10n, error));
  }
}
