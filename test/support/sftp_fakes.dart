...
final class FakeSftpFileSystem implements SftpFileSystem {
  ...
  Future<void> createDirectory(String path) async {
    ...
    listings[path] = [];
    _upsert(sftpParent(path), _baseName(path), 0, isDirectory: true);
  }
  ...
  void _store(String path, List<int> bytes) {
    contents[path] = bytes;
    _upsert(sftpParent(path), _baseName(path), bytes.length);
  }
...
/// 条目的名字取路径最后一段（根目录返回 `/`）；只服务假文件系统，
/// 生产侧不需要这个工具，故不放进 `sftp.dart`。
