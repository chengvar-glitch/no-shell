...
/// 密钥变更失败：connect 阶段抛密钥变更异常。
FakeTransport _keyChanged() => FakeTransport(
  error: const HostKeyChangedException(
    host: '10.0.0.9',
    port: 22,
    keyType: 'ssh-ed25519',
  ...
  group('会话层密钥变更归类与处置入口', () {
    ...
    testWidgets('普通失败不展示密钥处置按钮', (tester) async {
      final session = TerminalSession(
        server: _server,
        credentials: const SshCredentials(password: 'pw'),
        transport: FakeTransport(error: Exception('boom')),
        hostKeys: FakeHostKeyStore(),
      );
      addTearDown(session.dispose);
      await session.start();
