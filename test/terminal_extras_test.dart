...
/// 连接成功、可捕获「发往远端」内容的假传输；`lines` 为空即「还没输出过的会话」。
...
Future<TerminalSession> _connectedSession(FakeTransport transport) async {
...
void main() {
  ...
  group('会话日志入口（状态胶囊）', () {
    ...
    testWidgets('点状态胶囊打开会话日志，内容与终端画面一致', (tester) async {
      ...
      final manager = managerWith(store, _connected(), connect: true);
    ...
    testWidgets('空日志的「复制 / 保存」按钮禁用', (tester) async {
      final transport = _connected(lines: []);
      final store = ServerStore(seed: [_server]);
      addTearDown(store.dispose);
    ...
    testWidgets('没有会话时状态胶囊不可点', (tester) async {
      final store = ServerStore(seed: [_server]);
      addTearDown(store.dispose);
      final manager = managerWith(store, _connected(), connect: false);
      addTearDown(manager.dispose);
