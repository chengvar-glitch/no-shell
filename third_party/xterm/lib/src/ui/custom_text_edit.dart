...
class CustomTextEditState extends State<CustomTextEdit> with TextInputClient {
  ...
  // Mirror of the platform's last-reported text. Inserts are diffed against
  // this instead of the static [_initEditingState]: some embedders drop the
  // setEditingState() reset issued after an event (macOS on the composing
  // path), and GTK delivers one commit as a burst of identical updates
  // (commit + preedit-changed + preedit-end). Either way the next event
  // arrives as a cumulative TextEditingValue, and diffing against a stale
  // init re-emits characters that were already sent.
  ...
  void updateEditingValue(TextEditingValue value) {
    ...
    if (widget.deleteDetection) {
      // Placeholder-based delete detection (mobile soft keyboards): the
      // trailing placeholder is what keeps the IME emitting backspace
      // events, so this path keeps the static diff and the reset that
      // refills the placeholder.
    ...
    // Diff against the last text the platform reported, not against the init
    // state, so the math stays right even when the platform never applied a
    // previous reset. The post-event reset is dropped for the same reason:
    // forcing the platform back to init is what caused the divergence, and
    // letting its text accumulate keeps it in step with the mirror.
